...
public class Maze : MonoBehaviour {
	...
	void CreateWalls (){
		...
		for (int i = 0; i < ySize; i++) {
			for (int j = 0; j <= xSize; j++) {
				myPos = new Vector3 (initialPos.x + (j * wallLength) - wallLength / 2, initialPos.z + (i * wallLength) - wallLength / 2);
		...
		for (int i = 0; i <= ySize; i++) {
			for (int j = 0; j < xSize; j++) {
				myPos = new Vector3 (initialPos.x + (j * wallLength), initialPos.z + (i * wallLength) - wallLength);
	...
	void GiveMeNeighbour() {
		...
		if (length != 0) {
			int choseOne = 0;
			currentNeighbour = neighbours [choseOne];
			wallToBreak = connectingWall [choseOne];
		} else {
			if (backingUp > 0) {
